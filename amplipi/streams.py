...
class FilePlayer:
  ...
  def connect(self, src):
    ...
    print(f'{self.name} (stream: playing {self.url}) connected to {src} via {utils.output_device(src)}')
    self.state = 'connected'
    self.src = src
    # make a thread that waits for a couple of secends and returns after setting info to stopped
    self.bkg_thread = threading.Thread(target=self.wait_on_proc)
...
AnyStream = Union[Shairport, Spotify, InternetRadio, DLNA, Pandora, FilePlayer]
